...
bool WireguardUtilsLinux::addInterface(const InterfaceConfig& config) {
  ...
  if (code != 0) {
    logger.log() << "Adding interface failed:" << strerror(-code);
    return false;
  }
  ...
  if (wg_set_device(device) != 0) {
    logger.log() << "Failed to setup the device";
    return false;
  }
...
bool WireguardUtilsLinux::updatePeer(const InterfaceConfig& config) {
  ...
  logger.log() << "Adding peer" << config.m_serverIpv4AddrIn;
  ...
  if (!setPeerEndpoint(&peer->endpoint.addr, config.m_serverIpv4AddrIn,
                       config.m_serverPort)) {
    logger.log() << "Failed to set peer endpoint for hop" << config.m_hopindex;
    return false;
  }
  ...
  if (wg_set_device(device) != 0) {
    logger.log() << "Failed to set the new peer hop" << config.m_hopindex;
    return false;
  }
...
bool WireguardUtilsLinux::deletePeer(const QString& pubkey) {
  ...
  device->flags = (wg_device_flags)0;
  if (wg_set_device(device) != 0) {
    logger.log() << "Failed to remove the peer";
    return false;
  }
#endif
...
bool WireguardUtilsLinux::deleteInterface() {
  ...
  if (returnCode != 0) {
    logger.log() << "Deleting interface failed:" << strerror(-returnCode);
    return false;
  }
...
WireguardUtils::peerBytes WireguardUtilsLinux::getThroughputForInterface() {
  ...
  if (wg_get_device(&device, WG_INTERFACE) != 0) {
    logger.log() << "Unable to get stats for" << WG_INTERFACE;
    return pb;
  }
