...
bool Daemon::activate(const InterfaceConfig& config) {
  ...
  logger.log() << "Activating interface";
  ...
  if (supportWGUtils()) {
    ...
    if (!wgutils()->interfaceExists()) {
      if (!wgutils()->addInterface(config)) {
        logger.log() << "Interface creation failed.";
        return false;
      }
    }
    // Add the peer to this interface.
    if (!wgutils()->updatePeer(config)) {
      logger.log() << "Peer creation failed.";
      return false;
    }
  }
  ...
  if (supportWGUtils()) {
    for (const IPAddressRange& ip : config.m_allowedIPAddressRanges) {
      if (!wgutils()->updateRoutePrefix(ip, config.m_hopindex)) {
        logger.log() << "Routing configuration failed for" << ip.toString();
        return false;
      }
    }
  }
...
#define GETVALUE(name, where, jsontype)                         \
  if (!obj.contains(name)) {                                    \
    ...
    if (value.type() != QJsonValue::jsontype) {                 \
      logger.log() << name << " is not a " #jsontype;           \
      return false;                                             \
    }                                                           \
  ...
  if (config.m_deviceIpv4Address.isNull() &&
    ...
    logger.log() << "no device addresses found in jsonConfig input";
    return false;
  }
  ...
  if (config.m_serverIpv4AddrIn.isNull() &&
    ...
    logger.log() << "no server addresses found in jsonConfig input";
  }
  ...
  if (!obj.contains("dnsServer")) {
    ...
    if (!value.isString()) {
      logger.log() << "dnsServer is not a string";
      return false;
    }
...
bool Daemon::deactivate(bool emitSignals) {
  ...
  if (supportWGUtils()) {
    if (!wgutils()->interfaceExists()) {
      logger.log() << "Wireguard interface does not exist.";
      return false;
    }
    ...
    for (const ConnectionState& state : m_connections.values()) {
      ...
      logger.log() << "Deleting routes for hop" << config.m_hopindex;
      for (const IPAddressRange& ip : config.m_allowedIPAddressRanges) {
...
bool Daemon::switchServer(const InterfaceConfig& config) {
  ...
  logger.log() << "Switching server for hop" << config.m_hopindex;
